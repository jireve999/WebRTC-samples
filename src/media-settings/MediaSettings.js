...
export default class MediaSettings extends React.Component {
  ...
  initializeDevices = async () => {
    try {
      const devices = await this.updateDevices();
      this.initializeDefaultDevices(devices);
    } catch (error) {
  ...
  initializeDefaultDevices = (devices) => {
    const updates = {};
    
    if (!this.state.selectedAudioDevice && devices.audioDevices.length > 0) {
      updates.selectedAudioDevice = devices.audioDevices[0].deviceId;
    }
    
    if (!this.state.selectedVideoDevice && devices.videoDevices.length > 0) {
      updates.selectedVideoDevice = devices.videoDevices[0].deviceId;
    }
  ...
  soundMeterProcess = () => {
    if (!this.soundMeter) return;
    
    const val = (this.soundMeter.instant.toFixed(2) * 348) + 1;
    this.setState({ audioLevel: val });
    
    if (this.state.visible) {
      requestAnimationFrame(this.soundMeterProcess);
    }
  };
  ...
  getMediaStream = () => {
    const constraints = {
      audio: { deviceId: this.state.selectedAudioDevice ? { exact: this.state.selectedAudioDevice } : undefined },
      video: { deviceId: this.state.selectedVideoDevice ? { exact: this.state.selectedVideoDevice } : undefined }
    };
  ...
  handleDeviceChange = (type, value) => {
    this.setState({ [`selected${type}Device`]: value }, () => {
      setTimeout(this.startPreview, 100);
    });
  };
  ...
  renderDeviceSelector = (type, devices) => (
    <Select
      value={this.state[`selected${type}Device`]}
      style={{ width: 350 }}
      onChange={(value) => this.handleDeviceChange(type, value)}
    >
      {devices.map(device => (
        <Option value={device.deviceId} key={device.deviceId}>
          {device.label || `Unknown ${type} Device`}
        </Option>
      ))}
    </Select>
  );
        ...
        >
          <div className="item">
            ...
            <div className="item-right">
              {this.renderDeviceSelector('Audio', this.state.audioDevices)}
              <div 
          ...
          <div className="item">
            ...
            <div className="item-right">
              {this.renderDeviceSelector('Video', this.state.videoDevices)}
